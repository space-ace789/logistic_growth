...
ggplot(aes(t,N), data = growth_data) +
  ...
  labs(y = "Log(Population Size)", x = "Time", title = "Exponential Model of Bacterial Growth compared to a Logistic Model of Growth") +
  scale_y_continuous(trans='log10') +
  scale_color_manual(name='Growth Model',
                     breaks=c('Exponential', 'Logistic'),
                     values=c('Exponential'='blue', 'Logistic'='red')) +
  theme(plot.title = element_text(size = 9, hjust = 0.5), legend.position = c(0.3, 0.70))
